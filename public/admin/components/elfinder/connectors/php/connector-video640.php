...
$root = MEDIA_PATH.'/uploads/temp/' . $aid . '/events/640/';
$url = '/medias/uploads/temp/' . $aid . '/events/640/';

if (isset($_SESSION['events']) && isset($_SESSION['events']['action']) && $_SESSION['events']['action'] == 'add') {
    $root = MEDIA_PATH.'/uploads/temp/' . $aid . '/videos/640/';
    $url = '/medias/uploads/temp/' . $aid . '/videos/640/';
...
if (isset($_SESSION['events']) && isset($_SESSION['events']['eid']) && isset($_SESSION['events']['action'])) {
    if ($_SESSION['events']['action'] == 'edit') {
        ...
        $root = MEDIA_PATH.'/uploads/events/' . $eid . '/videos/640/';
        $url = '/medias/uploads/events/' . $eid . '/videos/640/';
    }
}

// Before opne elfinder, we should make sure folder exists.
if (!is_dir($root)) {
	try {
		$util_file = new Utils_File();
		$util_file->mkdirs($root);
	}
	catch(Exception $e) {
		//TODO:
	}
...
$opts = array(
	...
	//'URL'             => '/medias/uploads/events/', // root directory URL
        'URL'             => $url, //'/medias/uploads/temp/' . $aid . '/events/640/', // root directory URL
