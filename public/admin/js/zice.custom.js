...
(function($){
    $.fn.selectHierarchy = function(options) {        
        ...
        obj.after('<select class="drilldown-1 disable-selectmenu"><option value="">' + options.placeholder + '</option></select>');
        ...
        var change_handler = function(){
            ...
            if (node && node.children.length > 0) {
                this_select.after('<select class="disable-selectmenu"><option value="">' + options.placeholder +'</option></select>');
...
$(document).ready(function() {
    ...
    $("a.event-image-group").colorbox({
        rel: "event-image-group"
    });
...
//	$("ul.tabs li:first").addClass("active").fadeIn(400); 
    ...
    var dataTable2 = $('.data_table2').dataTable({
        "sDom": 'fCl<"clear">rtip',
        "sPaginationType": "full_numbers",
        "aaSorting": []
    });
    //dataTable2.fnSetColumnVis( 7, false );
...
$(function() {
    ...
    $('.submit_change_image_form').live('click', function() {
        ...
        $.confirm({
            'title': 'Alert',
            'message': "You are about to change an image loaded on the different Mobile Application. Are you sure to make those changes ?",
            'buttons': {
                'Yes': {
                    'class': 'special',
                    'action': function() {
...
//                            loading(str,overlay);
//                            setTimeout("unloading()",1500);
//                            setTimeout( "window.location.href='admin/users/login'", 2000 );
//                      }
//                  });
    });
    ...
    /*
     ...
     newImg = $('<img />').attr('src', '/medias/images/uploads/images/' + file.name);
     $('#target').append(newImg);
    ...
    $('.limit1m').limitInput({max: 1, disablelabels: true});

    // Select boxes
    $(function() {
        $('select').not("select.chzn-select, ,select[multiple], select#box1Storage,select#box2Storage").each(function () {
            var select = $(this);
            if (select.hasClass("disable-selectmenu")) {
    ...
    $('div.logout').hover(function() {
        ...
        $(this).find('img').animate({opacity: 0.4}, 200, function() {
            $(this).attr('src', '/public/admin/images/' + name + '.png').animate({opacity: 1}, 500);
        });
    }, function() {
    ...
    // on click callback
    $('#open_icon_teasers640, #open_icon_teasers720, #open_icon_teasers480, #open_icon_teasers320, \n\
            #open_icon_image640, #open_icon_image720, #open_icon_image480, #open_icon_image320,\n\
            #open_icon_video640, #open_icon_video720, #open_icon_video480, #open_icon_video320').click(function() {
        var callback = $(this).attr('id').substring(10);
        var type = $(this).attr('title');
        var input = $(this).attr('rel');
        fileDialogCallback(callback, type, input);
    });
...
function fileDialogCallback(callback, type, input) {
    ...
    $('<div id="finder_' + callback + '"/>').elfinder({
        url: '/medias/admin/components/elfinder/connectors/php/connector-' + type + '.php',
...
function countChars(message, counter, max) {
    var count = max - document.getElementById(message).value.length;
    if (count < 0) {
        alert("You have exceeded the max of 100 characters");
        document.getElementById(message).value = document.getElementById(message).value.substring(0, 100);
...
// Elfinder Interface
// @author jackey <jackey@c3o.asia>
(function ($) {
    $.fn.elfinderInterface = function (to, options) {
        var defaultOptions = {
            fileLimit: null,
        };
        ...
        // 初始化 token input
        to.tokenInput([]);
        //to.tokenInput("toggleDisabled", true);
        ...
        if (val.trim() != '') {
            ...
            for (var i = 0; i < val.length; i++) {
                var url = val[i];
                var parts = url.split('/');
                to.tokenInput('add', {id: url, name: parts[parts.length - 1]});
            }
        }

        // 初始化 elfinder
        var elfinder = dom.elfinder({
           url: dom.attr('href'),
           editorCallback: function (url) {
                var parts = url.split('/');
                // 如果文件已经添加了， 则不做处理了
                var k_v = to.tokenInput('get');
                for (var i = 0; i < k_v.length; i++) {
                    if (k_v[i] && typeof k_v[i]["id"] != 'undefined') {
                        if (k_v[i]["id"] == url) {
                            return false;
                        }
                    }
                }
                // 如果文件限制超过了，则把最前面的一个popup出来
                var val = to.val();
                var vals = val.split(',');
                var len = getLenOfToken(val);
                if (defaultOptions["fileLimit"] != null && len >= defaultOptions["fileLimit"]) {
                    to.tokenInput('clear');
                    for (var i = 1; i < vals.length; i++) {
                        var tmpurl = vals[i];
                        var tmpparts = tmpurl.split('/');
                        to.tokenInput('add', {id: tmpurl, name: tmpparts[tmpparts.length - 1]});
                    }
                }
                to.tokenInput('add', {id: url, name: parts[parts.length - 1]});
           },
...
// Language cookie switcher
(function ($) {
    $(function () {
        $("a.lang-code").click(function () {
            $.cookie('bo-lang', $(this).attr('code'), { expires: 7, path: '/admin' });
        });
    });
})(jQuery);
...
(function ($) {
    $(function() {
        $("button.page_add_button_remove").live('click', (function () {
            $(this).siblings(".page_attachment_is_been_removed").val(1);
            $(this).parent("dl").parent("fieldset").parent("dd").hide();
            var id = $(this).parent("dl").parent("fieldset").parent("dd").attr("id");
            var num = id.split("-")[0];
            $(this).parent("dl").parent("fieldset").parent("dd").siblings("#" +num + "-label").hide();
        }));
    });
})(jQuery);


// Kv on/ kv off button color select 

(function ($) {
    $(function () {
        var btn_on = $("#kv_on_btn_color");
        $("#kv_on_btn_color").ColorPicker({
            onSubmit: function(hsb, hex, rgb, el) {
		$(el).val(hex);
		$(el).ColorPickerHide();
            },
        ...
        var btn_off = $("#kv_off_btn_color");
        $("#kv_off_btn_color").ColorPicker({
            onSubmit: function(hsb, hex, rgb, el) {
		$(el).val(hex);
		$(el).ColorPickerHide();
            },
...
(function ($) {
    $(function () {
        $("#parent_id").selectHierarchy({ hideOriginal: true });;
    });
})(jQuery);