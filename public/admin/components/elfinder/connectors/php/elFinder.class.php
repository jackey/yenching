...
class elFinder {
	...
	public function run() {
		...
		header("Content-Type: ".($cmd == 'upload' ? 'text/html' : 'application/json;charset=utf-8'));
	...
	protected function _open()
	{
		if (isset($_GET['current'])) { // read file
			...
			header("Content-Disposition: ".$disp."; filename=".Utils_File::mb_basename($file));
	...
	protected function _paste()
	{
		...
		foreach ($_GET['targets'] as $hash) {
			...
			$_dst = $dst.DIRECTORY_SEPARATOR.Utils_File::mb_basename($f);
	...
	protected function _archive()
	{
		...
		foreach ($_GET['targets'] as $hash) {
			...
			$files[] = $f;
			$argc .= escapeshellarg(Utils_File::mb_basename($f)).' ';
		}
		$arc  = $this->_options['archivers']['create'][$_GET['type']];
		$name = count($files) == 1 ? Utils_File::mb_basename($files[0]) : $_GET['name'];
		$name = Utils_File::mb_basename($this->_uniqueName($name.'.'.$arc['ext'], ''));
	...
	protected function _extract()
	{
		...
		$cmd = $arc['cmd'].' '.$arc['argc'].' '.escapeshellarg(Utils_File::mb_basename($file));
		chdir(dirname($file));
		exec($cmd, $o, $c);
		chdir($cwd);
		if ($c == 0) {
	...
	{
		$rel  = $this->_options['rootAlias'] ? $this->_options['rootAlias'] : Utils_File::mb_basename($this->_options['root']);
		if ($path == $this->_options['root']) {
			$name = $rel;
		} else {
			$name = Utils_File::mb_basename($path);
	...
	protected function _info($path)
	{
		...
		$info = array(
			'name'  => htmlspecialchars(Utils_File::mb_basename($path)),
			'hash'  => $this->_hash($path),
		...
		if ($type == 'link') {
			...
			$info['linkTo'] = ($this->_options['rootAlias'] ? $this->_options['rootAlias'] : Utils_File::mb_basename($this->_options['root'])).substr($lpath, strlen($this->_options['root']));
	...
	protected function _tree($path)
	{
		$dir = array(
			'hash'  => $this->_hash($path),
			'name'  => $path == $this->_options['root'] && $this->_options['rootAlias'] ? $this->_options['rootAlias'] : Utils_File::mb_basename($path),
			'read'  => $this->_isAllowed($path, 'read'),
			'write' => $this->_isAllowed($path, 'write'),
			'dirs'  => array()
			);
	...
	{
                // 如果beforeRemove 返回false 则 不删除文件
                if (method_exists($this, "beforeRemove") && !$this->beforeRemove($path)) {
                    return ;
                }
	...
	protected function _path2url($path)
	{
		...
		$file = rawurlencode(Utils_File::mb_basename($path));
